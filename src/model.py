...
class CrossLingualToxicityTrainer:
    """
    Training utilities for the cross-lingual toxicity detector.
    Implements progressive GRL lambda scheduling.
    
    IMPORTANT: Handles unlabeled samples for cross-lingual training.
    For samples without toxicity labels (e.g., non-English data), use -1 as the label.
    The trainer will:
    - Skip toxicity loss computation for samples with label == -1
    - Always compute language loss for all samples (needed for GRL)
    - Only evaluate toxicity metrics on labeled samples
    
    This allows training with:
    - Labeled English data (toxicity labels 0 or 1)
    - Unlabeled multilingual data (toxicity label -1, language label 0-N)
    """
    def __init__(
        self,
        model,
        toxicity_criterion=None,
        language_criterion=None,
        toxicity_weight=1.0,
        language_weight=1.0
    ):
        self.model = model
    ...
    def compute_loss(self, outputs, toxicity_labels, language_labels):
        ...
        """
        # Create mask for labeled samples (toxicity_labels != -1)
        labeled_mask = toxicity_labels != -1
        
        # Toxicity loss (main task) - only for labeled samples
        if labeled_mask.any():
            # Get predictions and labels for labeled samples only
            labeled_logits = outputs['toxicity_logits'].squeeze(-1)[labeled_mask]
            labeled_targets = toxicity_labels[labeled_mask].float()
        ...
        # Language loss (adversarial task) - for all samples
    ...
    @staticmethod
    def get_grl_lambda(epoch, total_epochs, max_lambda=1.0):
        """
        Progressive GRL lambda scheduling.
        Lambda increases from 0 to max_lambda following the schedule from DANN paper.
        ...
        return max_lambda * (2. / (1. + torch.exp(torch.tensor(-10. * p))) - 1.)
